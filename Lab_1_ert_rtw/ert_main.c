...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Lab_1_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    Lab_1_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Lab_1_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Lab_1_M, "stopping the model");
  runModel = 0;
}
